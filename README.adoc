...
We will use a 2 byte header for each chunk to differentiate the chunks when reading and writing chunks into storage.

The 1st byte will be the last chunk flag. 1 for last chunk, 0 otherwise. 

The 2nd byte will be the chunk number.
